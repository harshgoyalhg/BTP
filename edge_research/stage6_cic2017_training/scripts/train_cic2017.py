...
def load_and_preprocess_cic2017(dataset_dir: str) -> pd.DataFrame:
    ...
    for f in csv_files:
        ...
        for chunk in pd.read_csv(f, chunksize=CHUNK_SIZE, low_memory=False,
            ...
            cleaned = pd.concat([feat_df, chunk['Label'].str.strip()], axis=1)
...
n_svm_lin = HP['svm_linear_n']
svm_lin_idx = np.random.choice(len(X_train_scaled), size=min(n_svm_lin, len(X_train_scaled)), replace=False)
X_svm_lin = X_train_scaled[svm_lin_idx]
y_svm_lin = y_train[svm_lin_idx]
...
np.random.seed(RANDOM_STATE)
n_svm_rbf = HP['svm_rbf_n']
svm_rbf_idx = np.random.choice(len(X_train_scaled), size=min(n_svm_rbf, len(X_train_scaled)), replace=False)
X_svm_rbf = X_train_scaled[svm_rbf_idx]
y_svm_rbf = y_train[svm_rbf_idx]

svm_rbf = SVC(**HP['svm_rbf'])
mt = MemoryTracker(); mt.start(); t0 = time.time()
svm_rbf.fit(X_svm_rbf, y_svm_rbf)
train_time = time.time() - t0; peak_ram = mt.stop()
logger.info(f"SVM-RBF done in {train_time:.2f}s | Peak RAM: {peak_ram:.2f} MB")
mp = os.path.join(MODEL_DIR, "svm_rbf.pkl")
joblib.dump(svm_rbf, mp)
evals = evaluate_model("SVM (RBF Kernel)", svm_rbf, X_val_scaled, y_val)
results_metrics.append({"Model": "SVM (RBF Kernel)", "Train Time (s)": train_time,
    "Inference Time (s)": evals["Inference Time"], "Peak RAM (MB)": peak_ram,
    "Model Size (MB)": os.path.getsize(mp)/(1024*1024),
    **{k: v for k, v in evals.items() if k not in ["CM","probs","preds","Inference Time"]}})
